...
trait TableTriat
{
     ...
     * @throws Exceptions
     */
    public function excuteQuery(string $type = 'all', bool $repository, array $data = null)
    {
        if ($type == "open") {
            ...
            if (isset($data['fields'])) {
                $entity = $this->typeQuery($type)->entityResult($data['fields']);
                if (!$repository) {
                    unset($entity->repository);
                }
                return $entity;
    ...
    public function entityResult(array $fields = null): object
    {
        ...
        $entities->tableRegistory($this->tableRegistory);
        ...
        for ($a = 0; $a < count($data); $a++) {
            ...
            if (count($newDatas) > 0) {
                $entities[$a] = $this->initializeEntity(
                    $table,
                    $this->typeQuery,
                    ['result' => $newDatas, 'fields' => $columns['fields']]
                )->entity($this->tableRegistory);
            }
        }
        return $entities;
    }
     ...
     * @return object
     */
    public function initializeEntity(string $table, string $type, array $options): object
    {
        $entity = $this->instance->resolveClass(
            ...
            ['config' => array_merge(['type' => $type], $options)]
        );
        if ($table != 'Entity') {
            $entity = $this->instance->resolveClass(
                $this->instance->namespaceClass(
    ...
    {
        $columns = !isset($fields) ? $this->query->getData($number, 'fields') : $fields[$number];
        if ($show) {
        ...
        foreach ($columns as $value) {
            if (strripos($value, " as ") !== false) {
                $value = substr($value, strripos($value, " as ") + 4);
            }
