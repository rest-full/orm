...
use Restfull\Datasourse\QueryModelInterface;
 ...
 */
class Assembly extends Query implements AssemblyInterface, QueryModelInterface
{
    ...
    {
        if ($this->existQuery()) {
            $generator = new Generator($this->getQuery());
        } else {
        ...
        if ($command[0] == "DML") {
            if ($command[1] == 'select') {
                $count = isset($this->data[$countData]['conditions']) ? $this->countConditions(
                    $this->data[$countData]['conditions']
                ) : 1;
                $querys = $this->amountSelect($count, $valid);
                $this->setQuery($querys, $countData, ['part' => false, 'sub' => false]);
            } elseif ($command[1] == 'insert') {
                $count = isset($this->data[$countData]['conditions']) ? $this->countConditions(
                    $this->data[$countData]['conditions']
                ) : 1;
                $querys = $this->amountInsert($count, $valid);
                if ($this->issetUnion || ($this->issetUnion && $this->issetNested)) {
                    $this->setQuery($querys, $countData, ['part' => true, 'sub' => false]);
                } elseif ($this->issetNested) {
                    $this->setQuery($querys, $countData, ['part' => false, 'sub' => true]);
                } else {
                    $this->setQuery($querys[0], $countData, ['part' => false, 'sub' => false]);
                }
            } else {
                $querys = $this->amountUdateAndDelete($valid);
                $this->setQuery($querys, $countData, ['part' => false, 'sub' => false]);
            }
        } else {
        ...
        if (count($generator->bindValue) > 0) {
            $this->setBindValues($generator->bindValue);
        }
        return $this;
    }
     ...
     * @throws Exceptions
     */
    private function amountSelect(int $count, array $valid): array
    {
        for ($a = 0; $a < $count; $a++) {
            $params = "";
            ...
            if (isset($this->data[$countData][$value])) {
                if ($value == "join") {
                    ...
                    if ($value == 'conditions') {
                        $conditions = '';
                        for ($a = 0; $a < $count; $a++) {
                            $conditions .= $generator->{$value}($newData, $a);
                            if ($a < $count) {
                                $conditions .= '),(';
                            }
                        }
